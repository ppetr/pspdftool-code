...
\noindent Abstrakt:  V~pøedlo¾ené práci se zabývám nìkterými tiskovými formáty (PDF, PostScript)~a
...
and abilities of modifiing already finished documents before printing.
In the other part of my work I focus on commonly used tools for preprinting preparations of documents.
Last part of my work is design and implementation of  tool for preprinting preparations of documents.\\
...
Pøíkladem tìchto úprav mù¾e být: zmìna rozmìrù stránky, selekce a pøeuspoøádání poøadí stránek nebo umístìní více stránek na jeden arch papíru.

\chapter {Dokumenty a práce s~nimi}

\section {Dokument}
Pod pojmem dokument rozumíme soubor na pamì»ovém médiu uchovávající nìjakou informaci. 
...
Pro vytvoøení a úpravu dokumentù se ve vìt¹inì pøípadù pou¾ívají patøièné editory (textové, grafické a dal¹í). Pokud lze dokument vytisknout pøímo z~editoru, obsahuje pøíslu¹ný editor funkci,
která vygeneruje z~upravovaného souboru  výstup do vhodného formátu pro tisk.
...
U~bitmapových formátù èasto chybí údaj o~rozmìru papíru, o~jeho orientaci, kvalitì tisku a podobné.
Dal¹ím nedostatkem formátu editoru je pøípadná závislost na konkrétním tiskovém prostøedí (rozli¹ení tiskárny, dostupná písma, a podobnì).
Napøíklad pro korektní zobrazení dokumentu z~OpenOffice je potøeba mít nainstalované øezy písma, která jsou vy¾adována danným dokumentem.

Formáty pro tisk, na rozdíl od formátù editorových, nejsou urèeny k~dal¹í editaci.
Tisknutelným formátem mù¾e být jazyk urèený pøímo pro tiskárnu (PostScript, PCL, GDI, HPGL).
Pøevod dokumentu na tento jazyk mù¾e mít jistá omezení, napøíklad  závislost na mo¾nostech konkrétním typu (rozli¹ení, maximální rozmìr papíru a dal¹í). 
Tisknutelným formátem mù¾e být také obecný formát obsahující informace urèené pro tisk (napøiklad PDF).
Aè formáty pro tisk nelze obecnì editovat, lze na nich provádìt nìkteré operace, které budou popsány dále.
V~nádledujícím textu se budu zabývat pøevá¾nì formáty PostScript a PDF.
...
Dùvodem pro pøedtiskové úpravy mù¾e být absence dané úpravy v~aplikaci, ve které dokument vznikl, nebo nedostupnost podkladù, 
...
Nìkteré ovladaèe tiskáren umo¾òují provést nìkteré z~tìchto úprav pøímo, ale v¾dy se jedná o~proprietární roz¹íøení konkrétního ovladaèe.
Proto je vhodné mít mo¾nost tyto úpravy nezávisle provést pomocí nìjakého nástroje.

\chapter{Vybrané formáty}
V~této kapitole budou popsány formáty PostScript a PDF,
které jsou vhodné pro tisk a je  na nich mo¾né realizovat nìkteré z~pøedtiskových úprav.
...
Pro jeho pøevod do rastrové grafiky se mù¾e pou¾ít GhostScript\cite{ghostscript}.

PostScript je turingovsky úplný programovací jazyk s~prefixovou notací.
Je podobný svou syntaxí jazyku Forth, ale na rozdíl od nìj je specializovaný pro popis grafického výstupu.
...
Poèátek tohoto systému je v~levém dolním rohu.  X-ová osa je orientována zleva do prava a y-ová osa vede zdola nahoru.
...
PostScript a PDF. Pro bod se souøadnicemi $[x,y]$ vypadá následovnì.
$$[x',  y',  1] = [x, y, 1]  \times
\pmatrix{
a & b & 0 \cr
c & d & 0 \cr
...
Jazyk PostScript a jeho DSC komentáøe jsou obsáhlej¹í, ne¾ je uvedeno v~této èásti, podrobnìj¹í informace je mo¾né nalézt ve specifikaci PostScriptu a DSC komentáøù.
...
Obsahuje pro ka¾dý nepøímý objekt v~souboru následující záznam: èíslo jeho revize a offset od zaèátku souboru, kde se objekt nachází.
...
Pøíkladem jeho pou¾ití mù¾e být vlo¾ení loga na stránku. Toto logo je reprezentováno XObjectem.
...
Pro podrobnìj¹í specifikaci PDF lze  doporuèit Adobe PDF reference Manual.


\chapter{Souèasné programy}
Tato práce se zabývá pøevá¾nì programy  PSUtils a pdftk. Byly zvoleny, proto¾e jsou dostupné a pracují s~formáty popisovanými v této práci. Dále existují m.j. FinePrint, Acrobat. Ty vzhledem k~jejich tì¾¹í dostupnosti (licence) dále nejsou zmiòovány. 
\section{PSUtils}
...
PSUtils umí vìt¹inu z~vý¹e zmínìných pøedtiskových operací. Nástroj pstops umí kombinovat následující základní operace: zmìnu velikosti stránky, rotaci stránky o~násobky 90 stupòù, posunutí stránky a spojení dvou stránek.
Umí spojit nìkolik dokumentù do jednoho, ale pouze takové, které mají stejný prolog.
Nelze v¹ak s~ním provést následující operace: zrcadlení stránek, kreslení èi psaní textu na stránku.

\section{Pdftk}
Jedná se o~nástroj\cite{pdftk} napsaný v~jazyce Java. Tímto programem je mo¾né provádìt následující operace: spojení více dokumentù do jednoho,
výbìr stránek z~dokumentu, otoèení stránek, dekryptovat PDF, enkryptovat PDF, pøedvyplnìní formuláøù, opravit po¹kozené
...
\chapter{Pspdftool}
Vý¹e zmínìné nástroje pro pøedtiskovou úpravu dokumentù se vzájemnì li¹í ve svém u¾ivatel\-ském rozhraní. Proto  byl vytvoøen univerzální nástroj,
...
Nástroj byl navr¾en s~ohledem na mo¾nost jeho roz¹íøení o~dal¹í formáty.
...
bude program provádìt na vstupní soubory. Jejich jména následují za pøíkazy. Posledním
argumentem je jméno výstupního souboru. Pokud je uvedeno více vstupních souborù, nejprve 
jsou spojeny do jednoho, jestli¾e to daný formát umo¾òuje.
...
výstupní soubor. Jednotlivé pøíkazy se mezi sebou oddìlují bílými znaky.\footnote{bílým znakem chápeme: mezeru, tabulátor, novou øádku}
\begin{center}
...
{\tt pspdftool -f cmds.txt in.pdf out.pdf } & na rozdíl od pøedchozího se budou èíst pøíkazy ze souboru cmds.txt\\
\end{tabular}
}
\vspace{5mm}
...
Parametry mohou být
následujících typù: celoèíselného, desetinného, rozmìru, øetìzce, nebo
...
Rozmìr obsahuje desetinný typ a identifikátor jednotky. Jednotkou mù¾e být:
...
Jednotlivé parametry pøíkazu jsou mezi sebou oddìleny èárkou. Mezi èárkou a parametrem pøíkazu mù¾e být posloupnost bílých znakù.
...
Poslední èástí je selektor stránek, který zaèíná levou a konèí pravou slo¾enou
...
\begin{tabular}{lp{8 cm}}
	...
	&\\
	{\tt foo(a)\{1..2 bar\}} & pøíkaz {\tt foo} s parametrem {\it a} a selektorem {\it1..2}, na který se aplikuje pøíkaz {\tt bar}\\
...
U~pøíkazu modulo udává první parametr n poèet stránek ve skupinách, na které se rozdìlí vstupní seznam. Pokud není celkový poèet stránek v~seznamu
...
a rozdìlí se na pøílslu¹né skupiny {\tt '1 2 3'} a {\tt '4 5 6'}. Vybereme {\tt \{1 -1\}} {\tt '1 6' a '4 3'} a výsledek spojíme {\tt '1 6 4 3'}.
Pokud parametr half je roven {\tt 1}, zpracovává se pouze polovina skupiny. V~tom pøípadì je výsledek pøedchozího pøíkladu {\tt '1 6'}.
...
Pøíkaz norm sjednotí velikost v¹ech stránek.
...
{\tt bbox scaleto(a4)} & pøepoèítá okraj a pøizpùsobí velikost stránek na rozmìr a4\\
...
V~jistých pøípadech je tøeba oèíslovat stránky, pro tento úkon je urèen pøíkaz number.\\
...
{\tt flip(mode)}& zrcadlovì otoèí v¹echny stránky v~seznamu, {\it horizontal{\tt |}h} vodorovnì, {\it vertical{\tt |}v} svisle. \\
{\tt line(b\_x, b\_y, e\_x, e\_y, size)}& nakreslí èáru ¹íøky {\it size} mezi body {\it b\_x, b\_y} a { \it e\_x, e\_y} \\
{\tt matrix(a,b,c,d,e,f)}& provede afinní transformaci souøadnic stránky dle zadané matice, význam jednotlivých argumentù viz. kapitola o~afinní transformaci\\
...
{\tt norm(center, scale)} & sjednotí velikost v¹ech stránek v~seznamu\\
{\tt number(x,y,start,text,font,size)}& pøidá  na jednotlivé stránky èísla stránek, {\it start}  
poèáteèní èíslo stránky, {\it font}  jméno fontu, {\it text}, který se má na stránku vykreslit, výchozí \uv{\%d} èíslo stránky (podobný formátu printf),  {\it size} velikost písma, v¹echny parametry jsou nepovinné\\
{\tt nup(x, y, dx, dy, orientation, frames)}& pøeskládá $x\times y$ stránek na jednu, {\it x} poèet stránek ve vodorovném smìru,
...
{\tt orient(landscape{\tt |}portrait)}&  nastaví orientaci textu na stránkách v~seznamu, neprovede se ¾ádná zmìna obsahu stránky, tato informace je dùle¾itá pro pøikaz nup, ovlivòuje zpùsob kladení stránek\\
...
{\tt read(name)} & pøipojí na konec seznamu  stránky ze souboru jménem {\it name}\\
...
{\tt write(name)}& ulo¾í seznam stránek do souboru jménem {\it name}\\
\end{longtable}

\subsubsection*{Omezení}
Soubory ve formátu PostScript nelze spojovat dohromady a jsou u nich vy¾adovány DSC komentáøe.
V pøípadì PDF formátu nelze pracovat s kryptovanými PDF. Program neumí pracovat s~dokumentem PDF,
který vyu¾ívá nìkterá roz¹íøení z~verze 1.5 a novìj¹ích.
Jedná se napøíklad o~ObjectStreams a Cross-Reference Streams.

\subsubsection*{Instalace}
\begin{enumerate} 
\item zprovoznit zlib a GhostScript\footnote{ zlib alespoò ve verzi 1.2.3, ghostscript alespoò ve verzi 8.60}
\item získat zdrojové kódy ze svn,\\
...
Program byl testován pod Linuxem,  Mac OS Xem, Solarisem (sparc), HP-UXem a AIXem. Pøedpokládá se snadná pøenositelnost i na ostatní UNIXové systémy, na kterých nebyl testován.

\chapter{Pspdftool - implemntace}
V~této kapitole se nachází popis jednotilvých èástí pspdftoolu. Program je napsaný v jazyce C.

\section{Zobecnìní dokumentu}
Dokument urèený pro tisk obsahuje informace o~stránkách a jejich vlastnostech. Pro reprezentaci stránek je zvolen spojový seznam. Jeho prvky odpovídají jednotlivým stránkám v~dokumentu.
...
Hlava seznamu obsahuje globální informace platné pro v¹echny stránky: maximální bbox, maximální velikost papíru, globální orientaci textu na stránce a implementaènì závislou èást na konkrétním formátu.
 ...
 Knihovna je rozdìlena na dvì èásti, èást nezávislou na konkrétním formátu a èást implementující konkrétní formát. Výhodou tohoto návrhu je snadná roz¹íøitelnost o~dal¹í formát.
 ...
 Èást závislá na formátu implementuje následující funkce: naètení dokumentu, ulo¾ení dokumentu, spojení dvou dokumentù, transformaci jednotlivých stránek, spojení dvou stránek do jedné, kreslení a psaní textu na stránku, oøez stránky.
 Zámìrem bylo navrhnout co mo¾ná nejjednodu¹¹í rozhraní mezi èástí na formátu závislou a nezávislou. Napøíklad rotace nebo zmìna velikosti stránky se provede aplikací vhodné transformaèní matice na stránku.
...
Pro tento pøípad je implementován formát {\it dummydoc}, jeho¾ interface je zavolán v~pøípadì absence urèité funkce konkrétního formátu. Ve vìt¹inì pøípadù skonèí toto volání chybou.
...
Pro ovìøení návrhu knihovny byly implementovány moduly pro následující formáty: PostScript a PDF. Tato implementace je také ukázkou, jak by se mohly pøidávat dal¹í moduly.

Ka¾dý modul implementující musí obsahovat funkci, která je pøedána funkci {\it doc\_register\_format} pro pøidání nového formátu do pspdftoolu. Pøi registraci modulu formátu je této  funkci pøedána struktura definující interface mezi závislou a nezávislou  èástí na formátu.
...
Za zmínku stojí implementace funkce, která nastavuje stránce transformaèní matici. Pokud je funkce nìkolikrát po sobì volána na stejnou stránku, 
nová matice se vynásobí s~aktuální. 

Ke ka¾dé stránce je pøiøazeno poèitadlo referencí, tím lze znaènì sní¾it spotøeba pamìti.
Pøi kopírování stránky se zvý¹í její poèítadlo referencí, obsah stránky se nekopíruje. 
V~pøípadì zmìny jejího obsahu se stránka zkopíruje a pùvodní stránce se sní¾í poèítadlo referencí.
Stránka se kopíruje pouze, pokud je potøeba ji zmìnit. 
Obdobný mechanismus je pou¾it i pro kopírování seznamù.

\subsection{PostScript}
Od~PostScriptového souboru je po¾adováno, aby obsahoval DSC komentáøe. Bez nich nelze urèit, ani¾ by bylo potøeba PostScript interpretovat,
...
Proto¾e PS soubor mù¾e být velmi veliký (stovky MB),  není mapován celý do pamìti. U~ka¾dé stránky se udr¾uje v~pamìti její offset v~souboru.
...
\subsection{PDF} 
U~PDF dokumentu je mo¾né implementovat v¹echny v~této práci zmínìné pøedtiskové úpravy. V~dokumentu se pøeètou v¹echny objekty. Objekty stránek se pøevedou na objekty XObject. K~pùvodní stránce se vytvoøí nová stránka, která obsahuje XObject z~pùvodní stránky. U~ka¾dé stránky v~seznamu je nutné si zapamatovat pøíslu¹né èíslo objektu, který ji reprezentuje. Pøi ukládání dokumentu se vygeneruje ze seznamu stránek
...
Operace typu transformace, spojení dvou stránek do jedné, kreslení èar a psaní textù se týkají streamu pøíslu¹né stránky. 
Nové stránky jsou vytváøeny z~odkazù na stávající XObjecty. Tyto stránky nemají zakódovaný content stream.
Není tedy potøeba implementovat jednotlivé filtry pro práci se streamem. Spojení dvou stránek odpovídá spojení dvou Content Streamù za sebe. Kreslení a transformace  odpovídá pøidání pøíslu¹ných operátorù do Content Streamu stránky.
Druhou výhodou daného øe¹ení je jednoduché slouèení resources stránky.

Problém nastane, jestli¾e je potøeba zkonvertovat stránku, její¾ popis je rozdìlen do více streamù. Pøímoèaré øe¹ení by bylo zkonvertovat ka¾dou èást na XObject.
...
slouèením více streamù do jednoho. Pro tento úkon je potøeba stream dekryptovat. Proto je program doplnìn o~funkce pro dekompresi streamu z LZW nebo deflate.
...
Pøi zápisu nového dokumentu se nejprve ze stránek v~seznamu vygeneruje vyvá¾ený strom a 
odstraní nepou¾ité objekty. Tato operace se provádí za pomoci prohledávání objektù do hloubky z~catalog objektu.
Po této operaci se zapí¹í objekty do souboru a vygeneruje se k~nim cross reference table. 
...
roz¹íøit o~dal¹í formáty, pøípadnì konverze jednoho formátu na druhý s~u¾itím
externího programu.

Na tisku by se dalo dále zkoumat mnohé, napøíklad: techniky tisku,
...
Dle mého názoru absence vý¹e zmínìných operací není omezující pøi tisku èernobílých dokumentù.


Dále by bylo mo¾né doplnit k~nástroji grafické rozhraní pro jeho intuitivnìj¹í ovládání.
...
Nový nástroj pspdftool, který je výsledkem této práce, je první obecný stránkovì orientovaný nástroj pro práci s~tiskovými formáty.
