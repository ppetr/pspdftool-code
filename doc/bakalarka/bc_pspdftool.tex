...
Pro jeho zobrazení se mù¾e pou¾ít GhostScript \cite{ghostscript}.
...
Tato práce se zabývá pøevá¾nì programy  PSUtils a pdftk,  z~dùvodu jejich dostupnosti. Dále existují následující programy: FinePrint, Acrobat, ale vzhledem k~jejich dostupnosti (licence) byly vybrány následující dva. 
\section{PSUtils}
...
První parametr n udává poèet stránek ve skupinách, na které se rozdìlí vstupní seznam. Pokud není celkový poèet stránek v~seznamu
...
Pro ovìøení návrhu knihovny byly implementovány následující formáty PostScript a PDF. Tato implementace je také ukázkou, jak by se mohly pøidávat dal¹í formáty.
...
Za zmínku stojí implementace funkce, která nastavuje stránce transformaèní matici. Pokud je funkce nìkolikrát posobì volána na stejnou stránku, 
nová matice se vynásobí s~aktuální. 

Pro sní¾ení pamìtových nárokù programu, pamatuji si u~ka¾dé stránky poèet referencí, které se na ní okdazují. 
Pøi kopírování stránky se zvý¹í poèítadlo referencí. Stránka se kopíruje pouze, pokud je potøeba ji zmìnit. 
...
Proto¾e PS soubor mù¾e být velmi veliký (stovky MB),  není nemapován celý do pamìti. U~ka¾dé stránky se udr¾uje v~pamìti její offset v~souboru.
...
Operace typu transformace, spojení dvou stránek do jedné, kreslení èar a psaní textu se týkají streamu pøíslu¹né stránky. 
...
Problém nastane, pokud je potøeba zkonvertovat stránku, její¾ popis je rozdìlen do více streamù. Pøímoèaré øe¹ení by bylo zkonvertovat ka¾dou èást na XObject.
...
slouèením více streamù do jednoho. Pro tento úkon je potøeba stream dekryptovat. Proto je program doplnìn o~funkce pro dekompresi streamu v LZW a deflate.
...
Nový nástroj pspdftool, který je výsledkem této práce, je první obecný stránkovì orientovaným nástroj pro práci s~tiskovými formáty.
