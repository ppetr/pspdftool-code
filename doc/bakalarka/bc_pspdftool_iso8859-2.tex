...
\chapter{Úvod}
V~této práci, \uv{Pøedtisková pøíprava dokumentù}, zkoumám mo¾nosti úprav dokumentù v~tiskovém formátu. Nezabývám se nástroji pro
...
Tøetí èásti je popis nìkterých souèasných nástrojù pro pøedtiskové úpravy dokumentù.
...
Mezi atributy popisující konkrétní stránky patøí: velikost papíru, orientace textu a Bounding Box.
...
Pro vytvoøení a úpravu dokumentu se pou¾ívá editor. Pokud je dokument urèený k~tisku, obsahuje pøíslu¹ný editor funkci,
...
Formáty pro tisk, na rozdíl od formátù editorových, nejsou urèeny k~dal¹ímu editováni, av¹ak nìkteré operace lze s~nimi také (jak uká¾eme dále) provádìt.
...
Pøevod dokumentu na jazyk pro tiskárnu mù¾e mít jistá omezení,napøíklad  závislost na konkrétním typu tiskárny. 
...
Pøedtiskové úpravy se týkají formátù explicitnì urèených pro tisk, patøí mezi nì pøeskupení, otoèení, selekce, zmìna formátu papíru, zvìt¹ení èi zmen¹ení stránek, ale nepatøí sem zmìna  obsahu,
...
jsou vhodné pro tisk a je  na nich mo¾né provést nìkteré z~pøedtiskových úpravy.

%Pøedtiskové úpravy by bylo mo¾né aplikovat také na formáty HPGL a PCL. 

\section{Grafický meta-formát PostScript}
PostScript \cite{postscript} je programovací jazyk. Byl navr¾en firmou Adobe.  Je primárnì urèen pro tisk dokumentù na laserových tiskárnách, 
...
Mezi základní elementy jazyka PostScript  patøí: oblouk, B\`ezeriova køivka a úseèka. Pro písmo existují speciální pøíkazy, které vykreslí písmena z~køivek.
...
User space je souøadnicový  systém, ve kterém se zadává poloha objektù (napø. èar).
...
Afinní transformace se aplikuje na jednotlivé grafické objekty. Touto transformací lze popsat v¹echny bì¾né transformace, jako je,
posunutí, otoèení nebo  zmìna velikosti. Lze jí reprezentovat pomocí transformaèní maticí $3\times3$. Tuto reprezentaci pou¾ívá 
PostScript a PDF.
...
Hlavièka konèí komentáøem {\it \%\%Komentáø v~PS souboru zaèíná znakem \% a konèí koncem øádky.
EndComments} nebo prázdným øádkem. V~hlavièce jsou uvedeny základní informace o~dokumentu, jako jsou: 
...
Vzhledem k~tomu, ¾e mohou být nìkteré èásti PDF komprimovány, je také vhodný pro pøenos dokumentù pøes sí» Internet.
...
Slovník streamu musí obsahovat povinný atribut {\it Length}, který udává délku bitového streamu. Nepovinný atribut {\it Filter} udává, jak je zakódovaný bitový stream.
Uvnitø streamu nemù¾e být odkaz na ostatní objekty v~dokumentu.

V~PDF je definován nepøímý objekt (indirect object). Nepøímým objektem mù¾e být libovolný objekt. Pro jeho jednoznaènou identifikaci jsou pou¾ita dvì èísla: první urèuje objekt, druhé urèuje jeho revizi, novìj¹í verze objektu má vy¹¹í èíslo.
Syntakticky nepøímý objekt vypadá následovnì: {\it èíslo objektu revize obj popis samotného objektu endobj }.
...
Objekt stránky je slovník. Obsahuje odkaz na {\it Content Stream}, popisující vzhled stránky, resources, pøes které se {\it Content Stream} stránky odkazuje na objekty v~dokumentu (napø. typ fontu), rozmìry papíru (MediaBox), rozmìr bounding boxu (TrimBox).
...
kreslení èi psaní textu na stránku. O~tyto operace lze nástroj snadno roz¹íøit. PSUtils umí spojit nìkolik dokumentù do jednoho, ale pouze takové, které mají stejný prolog.
...
\begin{tabular}{lp{8 cm}}
	...
	{\tt pspdftool "foo" in.pdf out.pdf}  & naète soubor in.pdf aplikuje na seznam stránek pøíkaz foo a výsledek ulo¾í do souboru out.pdf (uvozovky kolem pøíkaz jsou kvùli shellu)\\
...
{\tt nup(2,2,paper=a4)} & je ekvivalentní pøíkazu nup(4) akorát výsledná stránka má formát a4\\
\end{tabular}
\vspace{5mm}
...
dx,dy (minimalni ¹íøka mezi stránkami), orient (postup kladeni stránek na stránku, portrait zleva doprava, s hora dolu, landscape zprava doleva z hora dolu).
Pokud není orient nastaven, bere se jako výchozí informace ze vstupního seznamu.
Velikost výsledné stránky se nezmìní, pokud není uvedeno jinak.


Daksi u¾iteènou operací nu¾e být pøerovnání stránek pro tisk kní¾eèky.
...
\vspace{5mm}
\begin{tabular}{lp{10 cm}}
{\tt scaleto(a4)}&zvìt¹ení/zmen¹ení velikosti papíru na a4\\
{\tt scaleto2(1,2,3,4)}&	zvet¹eni/zmen¹eni stránky na explicitnì zadanou velikost (levý dolní, pravý horní roh)\\
{\tt paper(a4)}&nastaveni velikosti papíru a a4  (neprovádí se zvet¹eni, zmen¹eni stránky)\\
{\tt crop(a4)}&oøíznuti stránky na daný formát\\
{\tt crop(1,2,3,4)}&oøíznuti stránky na daný rozmìr\\
...
násobkem èísla n, pøidá se na konec seznamu pøíslu¹ný poèet prázdných stránek, aby seznam mìl délku rovnu násobku èísla n. Pokud je nastaven parametr round, tak n je rovno  max(n,round).
...
která je zrcadlovì symetrická dle støedu seznamu. Výsledná skupina seznamu je spojena do výstupního.
\subsubsection*{priklad:}
Nech» máme seznam '1 2 3 4 5 ' aplikujme na nìj modulo s n rovným 3. Nejprve se seznam doplní na vhodnou delku '1 2 3 4 5 6 ' 
a rozdìlí se na pøílslu¹ne skupinky '1 2 3' a '4 5 6'. Vybereme {1 -1} '1 6' a '4 3' a výsledek spojíme '1 6 4 3'.
Pokud parametr half je roven 1, zpracovava se pouze polovina skupinek, pak výsledek pøedchozího prikladu je '1 6'.
...
{\tt modulo(2)\{1 2 rotate(180)\}} &	odpovídá pøíkazu duplex(1), tj. první stránka bez zmìny, druha se rotuje o 180 stupòù\\
{\tt modulo(16)\{1..16 book\}} & vytvoøení signatury o 16-ti stránkách pro vázání kní¾ek\\
\end{tabular}
\vspace{5mm}

Pøíkaz apply provede na  vybraných stránkách zvolené pøíkazy. Pøíkaz select vybere stránky ze seznamu a pøípadné na nì aplikuje pøíkazy.
...
Mezi jeho argumenty patøí center, výchozí hodnota je centrovaní podle os x a y, none vypne centrovaní, x centrovaní podle osy x, y centrovaní podle osy y a 
xy centrovaní podle obou os.

Nìkdy mají stránky zbyteènì velké okraje. V~tomto pøípadì je mo¾né nechat GhostScript pøepoèítat jejich okraje.
Tato operace se provádí pomocí pøíkazu bbox. Rozmìry výsledného okraje je maximum z~okrajù jednotlivých stránek.

\vspace{5mm}
\begin{tabular}{lp{10 cm}}
{\tt bbox scaleto(a4)} & pøepoèítání okrajù a pøizpùsobení velikosti stránek na papír a4.\\
{\tt bbox norm(center=y) scalet(a4)} & pøepoèítání okrajù a pøizpùsobení velikosti stránek na papír a4, vhodné pro stránky se zrcadlovì orientovanými okraji. \\
\end{tabular}
\vspace{5mm}

V~jistých pøípadech se mu¾e hodit oèíslovat stránky, pro tento úkon je urèen pøíkaz number.\\

\vspace{5mm}
\begin{tabular}{lp{10 cm}}
{\tt numer} &	oèísluje v¹echny stránky, zaèíná se èíslem 1 a je umístìno dolu na støed stránky\\
{\tt number(10,10)} & obdobnì jako pøedchozí pøípad, akorát je èíslo umístìno na pozici 10, 10 od levého dolního rohu.\\
...
{\tt cmarks(by\_bbox)}&pøidá na stránky oøezové znaèky, pokud je by\_box roven 0 pøidávají dle rozmìru stránky, v opaèném pøípadì dle bounding boxu\\
...
{\tt flip(mode)}& zrcadlení v¹ech stránek v~seznamu, {\it horizontal{\tt |}h} vodorovnì, {\it vertical{\tt |}v} svisle. \\
...
{\tt paper(paper\_size)}& nastaví velikost papíru stránkám v~seznamu, neprovádí se ¾ádná zmìny mìøítka\\
{\tt paper2}(x, y)& nastavení explicitních rozmìrù papíru\\
...
{\tt scale(scale)} &	zmìní velikost stránek v zadaném mìøítku <1 zmen¹ení, >1 zvìt¹ení\\
{\tt scaleto(paper, top, right, bottom, left)} &zmìní velikost v¹ech stránek v~seznamu, {\it paper\_size} velikost papíru na kterou se stránka pøetransformuje, okraje jsou explicitnì nastaveny na pùl palce\\
{\tt scaleto2(x, y, top, right, bottom, left)} &zmìna velikosti stránky na explicitní velikost papíru, provádí se zmìna mìøítka\\
{\tt select\{page\_ranges\}}& vybere stránky dle {\it seznamy\_stránek}, aplikuje  na nì pøíslu¹né pøíkazy, výsledný seznam obsahuje stránky z~výbìru\\
...
V pøípadì PDF formátu nelze pracovat s kryptovanými PDF. Dále neumí program pracovat s PDF,
která vyu¾ívají nìkterá roz¹íøení z verze 1.5 a novìj¹ích.
Mezi nì napøíklad patøí ObjectStreams a Cross-Reference Streams.
...
Program funguje pod Linuxem a Mac OS X. Mìl by fungovat i na jiných rozumných UNIXech. Je závislý na zlibu a GhostScriptu.
...
U~stránky si pamatuji, kde se nachází její kód ve zdrojovém souboru, proto¾e PS soubor mù¾e být velmi veliký (stovky MB), není rozumné jej naèíst celý do pamìti.
...
Vzhledem k tomu, ¾e mohou být streamy mezi sebou závisle, nemusí být tento pøevod korektní ve v¹ech pøípadech. Tento problém lze vyøe¹it pouze
...
Mezi jeho hlavní výhody patøí: stejné rozhraní pro více formátu, mo¾nost
